...
import { findUserById, findUserByIdentifier, isUserActive, updateUserActive } from "../prisma/userService";
import { findUserLibrary, updateUserLibraryActiveStatus } from "../prisma/userLibraryService";
...
export const toggleUserActiveByLibrarian = async (
    ...
    try {
        ...
        const userLibrary = await findUserLibrary(user.user_id, librarianLibraryName);
        if (!userLibrary) {
        ...
        await updateUserLibraryActiveStatus(identifier, librarianLibraryName, isActive);
